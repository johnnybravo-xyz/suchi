...
//	Resolve   — mark an approval task done, enqueue advance.
...
var ErrEngineNotConfigured = errors.New("approvals: default engine not configured")
...
// Resolve marks an approval task as resolved with the chosen option,
// then enqueues an approval:advance job so the run advances. Actor
// must be the assignee or an admin (checked by caller).
