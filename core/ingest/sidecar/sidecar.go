...
// shape (versioned via `suchi_sidecar: 1`) and the Bundle-native
// shape emitted by producers like an-existing-dms post-consume scripts
// and johnnybravo-xyz/mail-intake — flat JSON with `title`, `created`,
// `correspondent`, `tags`. The compat path lets you drop suchi into
// an existing Bundle-style ingest chain without patching the
// upstream producer.
//
...
func Parse(b []byte) (*V1, error) {
	...
	// Compat: no version key → assume Bundle-native flat JSON.
	...
	// consumers.
	if s.Version == 0 {
		if !looksLikeBundleSidecar(b) {
			return nil, errors.New("sidecar: missing suchi_sidecar version and no recognized Bundle-style keys")
		}
		s.Version = Version
		// Bundle-native `correspondent` may be a "Name <email>"
...
// looksLikeBundleSidecar is the cheap sniff for "this JSON came
// from a Bundle-compatible producer, not from a broken suchi one."
// True when any of the well-known Bundle-native keys is present.
func looksLikeBundleSidecar(b []byte) bool {
