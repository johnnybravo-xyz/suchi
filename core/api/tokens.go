...
//
// Sibling to /api/token/ (singular), which is the mobile-compat
// credential-exchange endpoint (username+password → token). /api/tokens/
...
func (s *Server) CreateToken(w http.ResponseWriter, r *http.Request) {
	...
	if scopes == "" {
		scopes = auth.ScopeDocumentsRead + "," + auth.ScopeDocumentsWrite
	}
	...
	// Same wire shape as /api/token/ so mobile clients that switch
	// endpoints don't re-implement the response parse.
...
// scopesSubset reports whether every scope in `child` is present in
// `parent`. Legacy wildcards ("read", "write") get expanded on the
// parent side so a mobile-flow token can still mint tokens that
// carry granular scopes — matching the auth chain's HasScope rules.
func scopesSubset(child string, parent []string) bool {
	...
	// Wildcard expansion.
	if parentSet["write"] {
		parentSet[auth.ScopeDocumentsRead] = true
		parentSet[auth.ScopeDocumentsWrite] = true
		parentSet[auth.ScopeAgentTasks] = true
		parentSet[auth.ScopeAdminWebhooks] = true
	}
	if parentSet["read"] {
		parentSet[auth.ScopeDocumentsRead] = true
	}
...
// Sentinel — makes go vet + linters happy when the file grows methods
// that never allocate DB rows in the zero-value test.
var _ = errors.New
var _ = sql.ErrNoRows
