...
func TestDocumentDetailExposesDeviceOCRProvenance(t *testing.T) {
	...
	if _, err := d.ExecWrite(context.Background(), `
		UPDATE documents
		SET content = 'server text', content_source = 'server', encryption_state = 'decrypted'
		WHERE id = 1
	`); err != nil {
		t.Fatal(err)
	}
	...
	if detail.Content != "server text" || detail.ContentSource != "server" || detail.EncryptionState != "decrypted" ||
	...
	if metadataDetail.Content != "" || metadataDetail.ContentSource != "server" || metadataDetail.EncryptionState != "decrypted" ||
