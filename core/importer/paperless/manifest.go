// Package paperless imports a Paperless-ngx export bundle.
//
...
//     same bundle re-imports nothing (paperless_id_legacy is UNIQUE).
...
//     bytes will dedup in the CAS layer even if Paperless kept both.
//
...
func LoadManifests(root string) (Manifest, error) {
	...
	// Newer Paperless writes them under `documents/`; older versions
...
func trimSpaces(b []byte) []byte {
	// Paperless serializations are UTF-8 with occasional BOM. Strip
...
type CustomFieldFields struct {
	Name      string          `json:"name"`
	DataType  int             `json:"data_type"` // Paperless enum
...
type CustomFieldInstance struct {
	...
	// value is polymorphic in Paperless; we keep the raw and decode per-type at write time.
...
type NoteFields struct {
	...
	Created  string `json:"created"` // Paperless writes ISO8601
}

// DocumentFields is the main event. Paperless serializes many optional
...
// files on disk relative to the bundle root. Paperless names files
...
// ParseTime accepts Paperless's ISO8601 variants and returns a Unix
