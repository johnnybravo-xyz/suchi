...
// live suchi row for a given bundle_id_legacy. Only field NAMES; the
...
func Verify(ctx context.Context, d *db.DB, log *slog.Logger, opts VerifyOptions) (*VerifyReport, error) {
	...
	for _, o := range objs {
		...
		err := d.Read.QueryRowContext(ctx, `
			SELECT title, original_size FROM documents WHERE bundle_id_legacy = ?
		...
		if len(diff) == 0 {
			...
			rep.Differ = append(rep.Differ, DocDiff{BundleID: o.PK, Fields: diff})
		}
	}

	// Orphans: suchi docs with a bundle_id_legacy that's not in this bundle.
	rows, err := d.Read.QueryContext(ctx, `
		SELECT bundle_id_legacy FROM documents
		WHERE bundle_id_legacy IS NOT NULL AND trashed_at IS NULL
	`)
	if err != nil {
		return nil, err
	}
