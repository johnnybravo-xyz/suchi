...
func tokenRouteScopes() map[string]string {
...
func tokenScopeResolver(mux *http.ServeMux) httpx.TokenScopeResolver {
	...
	return func(r *http.Request) (string, bool) {
		...
		pattern := tokenRequestPattern(mux, scopes, r)
		scope, allowed := scopes[pattern]
		if !allowed {
			return "", false
		}
...
func tokenRequestPattern(mux *http.ServeMux, scopes map[string]string, r *http.Request) string {
	if pattern := exactTokenPattern(mux, scopes, r); pattern != "" {
		return pattern
	}
	...
	r2.URL.RawPath = ""
	return exactTokenPattern(mux, scopes, r2)
}

func exactTokenPattern(mux *http.ServeMux, scopes map[string]string, r *http.Request) string {
	_, pattern := mux.Handler(r)
	if _, allowed := scopes[pattern]; !allowed {
		return ""
	}
	path := pattern
