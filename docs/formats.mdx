...
### Additional OCR languages (TODO)

The default is English (`eng`). Selecting another language does not install
its Tesseract data. For example, Hindi and English text needs both language
packs and `OCR_LANGUAGES=eng,hin` (or the equivalent OCR language preference).
...
- Document and verify adding language packs to both Docker targets and
  bare-metal installations, without silently downloading them at runtime.
- Add synthetic image and PDF regression tests for mixed-script extraction,
  starting with Hindi + English, including missing-pack behavior and rescans.
- Deployment evaluation of local multilingual engines remains pending.
  ...
  is the first candidate: its Devanagari recognizer supports Hindi and English
