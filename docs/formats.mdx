...
the canonical `original_blob` and downloads retain the
...
Common raster images are wrapped in a one-page PDF before OCR and also pass
through the in-process barcode decoder. Other image formats receive the
barcode pass only.
...
Pick full only when OCRmyPDF must write a text layer into scanned PDFs. On
amd64, the measured compressed sizes are about 70 MB and 155 MB respectively.
