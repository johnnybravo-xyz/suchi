---
title: "Comparison"
description: "Honest side-by-side vs an existing DMS, Papra, and docspell — features suchi has, features they have, what's on the roadmap, what's out of scope."
---
...
| **an existing DMS** | The reference OSS DMS. Django + Redis + Postgres + Celery. Ten years of polish. | GPL-3.0 |
...
- **suchi** is fighting for "boring underneath, easy to get around."
  ...
  the pitch.
- **an existing DMS** is fighting for feature completeness and
...
| Feature | suchi | an existing DMS | Papra | docspell |
| ------- | :---: | :-----------: | :---: | :------: |
| Upload API | ✅ | ✅ | ✅ | ✅ |
| Watched folder | ✅ | ✅ | ✅ | ✅ |
...
| Mobile-app wire compat (bundle-mobile / swift-bundle) | 🚧 | ✅ | ➖ | ➖ |
...
| Signed images + SBOM | ✅ | ➖ | ➖ | ➖ |
| Import from an existing DMS | ✅ (`suchi import bundle`) | (self) | ➖ | ➖ |
| Audit log | ✅ | ➖ | ➖ | ➖ |
...
- **Community.** an existing DMS has years of accumulated Reddit,
...
- **Mobile compat green.** The endpoints exist, but the
  ...
  yet. an existing DMS is what the apps target natively.
- **UI depth.** an existing DMS's Angular UI has features suchi
  doesn't (bulk edit, saved-view editor UI, drag-drop custom-field
  editor). Suchi's UI is intentionally thinner.
- **Ecosystem plugins.** Bundle has more community-maintained
...
- **an existing DMS → suchi**: `suchi import bundle <dir>` is a
  first-class product feature. See [importer](/importer).
- **suchi → an existing DMS**: no shipped export tool. The DB is
  SQLite; the schema is documented. You'd write a script.
- **suchi → docspell / Papra**: same as above.

If unsure between suchi and an existing DMS and you already have data
in an existing DMS, the import path being one-way in the other
