...
| **Approvals** | `core/workflow` | `/api/approvals/` | Human-in-the-loop state machines: doc 17 needs manager sign-off, then finance, escalating on timeout. See [approvals](/approvals). |
...
| PATCH | `/api/automations/{id}` | Replace triggers and actions wholesale. |
