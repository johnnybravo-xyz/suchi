...
Calendar entry keeps its source document and the exact text that produced the
date, so you can verify it instead of trusting an unexplained event.
...
Configure the optional model and choose **Test connection**. After the test
succeeds, the **Choose what Suchi can handle automatically** section keeps the
controls in two clearly labeled boxes:

- **Works without a model** contains the two similar-document matching sliders.
- **Uses the configured model** contains **Apply model suggestions at** and
  **Add high-confidence dates to Calendar automatically**.

Choose the options, then select **Save model and options**. The model suggestion
score is the threshold Suchi uses for model-proposed filing details and dates.

When automatic Calendar dates are on:

- dates meeting the confidence score go directly to Calendar;
- lower-confidence dates wait in Approvals; and
- enabling it or lowering the score also moves matching pending dates to
  Calendar without another model call.

When it is off, every newly extracted date waits in Approvals. Turning it off or
raising the score never removes dates already in Calendar.
...
They also affect documents you explicitly rescan.
