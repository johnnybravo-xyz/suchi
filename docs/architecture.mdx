...
cancellation, then calls `app.Run(ctx, app.Options{...})`. The importable package
returns errors; it neither parses command arguments nor exits the process.
...
5. Construct the durable dispatcher, core subscribers, intake supervisors,
   schedulers and API routes without starting their loops.
...
`app.Options` contains resolved `config.Config`, logger, build identity, migration
sets, additional action definitions and one optional `Configure` callback. Its
concrete `Services` value exposes only `DB`, `Mux`, `Jobs`, `Approvals`, `Actions`
and `Log`, allowing a compiled distribution to register native HTTP routes and
durable subscribers. Unlisted routes remain session-only under the shared token
policy. Zero extension options preserve the community vocabulary and do not
create extension ledger state. This is compiled composition, with no dynamic
loader, service lookup, CAS/API exposure or registration framework.
...
`0003_taxonomy_foundation.sql`. The final step creates the one-row
`schema_lineage = 'final-beta-schema-3'` marker used for stable adoption;
`user_version` remains 3. Stable v1 starts a new migration epoch with one squashed
baseline for fresh databases. Its guarded compatibility path accepts published
beta.2/schema 2 by applying the frozen 0003 transition before adoption, and accepts
beta.3/schema 3 by adoption alone. Both happen within one v1 startup; beta SQL is
not part of the active stable migration list. Intermediate development schemas
remain unsupported. Validate all versions before touching the database; reject
duplicates/nonpositive versions with names.
...
verify rollback/writer enforcement. The v1 baseline work must separately prove the
beta.2/beta.3 adoption matrix. Runtime compatibility is a separate gate; see
...
across account creation, shares, login, and token exchange. Login takes a slot
before the account lookup, so overload fails quickly and identically for known
and unknown accounts while limiting Argon2 working memory.
...
- **Browser origin:** cookie mutations reject sibling-origin `same-site` and
  `cross-site` Fetch Metadata requests. Headerless anonymous form logins require a
  same-host `Origin`; headerless JSON credential routes require
  `application/json`. Authenticated headerless compatibility remains, and explicit
  token/bearer integrations are exempt. CORS alone is insufficient.
...
sweep handles only due workflow timeouts; it does not resolve filing suggestions.
Document-change projections and resolutions recheck current source, field and
access state, leaving stale proposals read-only until dismissed. See
...
confirmed Blank and taxonomy Apply satisfy the same filing-tree invariant. The
browser reads this projection through `GET /api/admin/setup/state`; the only
