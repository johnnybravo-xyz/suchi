...
| Portable taxonomy | `core/jd/presetfile` validates/normalizes HuML/TOML; `core/jd/importer` previews/applies; `core/taxonomy` exports |
...
reports actual scopes/build identity. `/api/token/` creates tokens, `/api/login`
creates sessions; mobile reads use existing ACL-filtered API projections.
...
Local matching fills unresolved metadata before deterministic user automations.
`core/automations` owns triggers/conditions/actions; `core/approvals` owns human
state machines. Integrations must not add another workflow engine. Advancement
jobs carry system/transition revision; handlers run outside the writer, then their
transactional effects/tasks recheck revision/status. Tasks retain originating
revision so retries cannot reopen resolved visits. Resolution clears deadlines;
...
Unrelated systems do not stale a plan; only safe initial setup replaces bootstrap,
later imports merge, and refile stays explicit.

`core/taxonomy` exports consistent `archive` revision-1 snapshots. Tree-only omits
