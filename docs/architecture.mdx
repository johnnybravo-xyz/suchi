...
embedded, numbered, and applied at boot. Each release adds at most one migration:
fold unreleased schema changes into that file, then keep it immutable after
publication. Later schema corrections use a new migration.
...
Ready jobs drain in serial batches; the polling interval is an idle fallback,
so a backlog or follow-up stage does not wait between batches.
...
Cookie-session mutations accept same-origin and headerless compatibility
requests but reject both sibling-origin `same-site` and `cross-site` browser
requests through Fetch Metadata checks. Token-authenticated integrations are
exempt. UI responses set CSP, frame, and content-type protections.
