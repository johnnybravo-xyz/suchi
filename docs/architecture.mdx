...
The existing approvals engine is also passed explicitly to `api.Server.Approvals`
and its subscriber. This replaces the former process-global default solely to
keep independently assembled instances isolated; state-machine handlers, review
validation, audit and retry logic remain with `core/approvals`.
...
`schema_lineage = 'final-beta-schema-3'` marker required for guarded v1 adoption;
`user_version` remains 3. Supported inputs are fresh databases and published
