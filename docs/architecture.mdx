...
- **Automations** (`core/automations/`) — trigger→conditions→actions
  ...
  an existing DMS's "workflows" shape in the mobile-compat plane but
  named for what it does. See [automations](/automations).
- **Approvals** (`core/workflow/`) — human-in-the-loop state
...
route at boot: `/`, `/docs/{id}`, `/preview/{id}`, `/download/{id}`,
`/pending-decryption`, `/upload`, `/inbox`, `/login`, `/bootstrap`,
`/admin/*`. What stays: `/api/*` (the JSON surface),
`/healthz`, `/readyz`, `/metrics`, and `/assets/*` (kept so an
external SPA can still fetch the favicon + manifest if it wants).

Intended for deployments where a separate frontend — an SPA built
against `/api/`, an internal-tools dashboard, a mobile app — is
the primary UX. suchi runs as an API server; the frontend is
whatever the operator hosts.
