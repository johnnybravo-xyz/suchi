...
`tessocr` engine (pdftoppm → tesseract). Image barcodes and EPUBs
work out of the box. **What it doesn't ship:** the ocrmypdf wrapper
(so no searchable-PDF archive for scanned docs — the extracted text
still lands in `documents.content` and FTS indexes it), DjVu, and
the planned Office-doc converter.

For those, pull the **full** image (~1 GB):
