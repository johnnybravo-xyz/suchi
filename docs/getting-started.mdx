...
(~30 MB, distroless). It handles image uploads (barcode decode) and
EPUB out of the box, but PDFs land with empty content unless the
document already has a text layer that pdf-inspector can extract —
which, since `pdftotext` isn't in the slim image, it can't.

For the full ingest chain (real qpdf, pdftotext, ocrmypdf, djvutxt,
ghostscript) pull the **full** image: `ghcr.io/johnnybravo-xyz/suchi:latest-full`
(~1 GB). It's the same binary; only the runtime dependencies differ.

See [supported file types](/formats) for the per-format matrix.
