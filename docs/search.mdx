...
| Omnibox completion | Active final query token | Prefix vocabulary order | Complete filters without running Search. |
...
6. Returns at most six source snippets of about 80 FTS tokens.
7. Reauthorizes up to three cited context sources from the previous turn.
...
can still be a rich query, category, current document, or exact document set;
the scope is ANDed with the retrieval candidates.
