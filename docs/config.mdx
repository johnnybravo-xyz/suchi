...
| `OCR_LANGUAGES` | `eng` | Comma-separated tesseract lang packs handed to every OCR call. Keep this to the 2–3 languages you actually receive — Tesseract slows non-linearly past ~3 packs and each additional pack costs RAM at load time. Common Indian scripts: `hin`, `kan`, `tam`, `tel`, `ben`, `mar`, `guj`, `pan`, `mal`, `urd`. Detected per-doc language (see [architecture § Languages](/architecture#languages)) is stored on `documents.languages`; it does not feed back into `OCR_LANGUAGES` in v1 — that's the operator's call. |
