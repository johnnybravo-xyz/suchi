...
| Var            | Meaning                                                                                 |
| -------------- | --------------------------------------------------------------------------------------- |
| `DOC_ID`       | Installation-global numeric document ID; not a category-local number or filing address. |
...
Tags are resolved or created in the document's filing system. Custom-field names
resolve only there; unknown names are logged and skipped, so create the definition
in that system first through the admin surface. The envelope cannot select a
system or transfer the document. Actorless document-link fields may refer only
to live documents in the same system.
...
- This is an instance-wide operator hook, not a per-system security sandbox.
  Its operating-system access can span all systems; the operator remains trusted.
