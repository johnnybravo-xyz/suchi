...
Taxonomy objects (`tag`, `correspondent`, `document_type`,
`storage_path`) have grant CRUD wired but not yet enforced on their
own handlers. That's a follow-up batch — the schema and API surface
are stable so scripts can start creating grants today.
