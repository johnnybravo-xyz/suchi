---
title: "Workflows"
description: "State-machine engine for review chains, approvals, and timeout-driven escalation — driven by durable-outbox jobs."
---

Workflows sit next to [agents](/agents). Agents are horizontal work
injection — anything speaking HTTP claims a job. Workflows are ordered,
resumable review chains: doc 17 needs manager sign-off, then finance
sign-off, escalating on timeout.

The design is a hybrid state-machine + job-graph. Each run keeps one
`current_state` cursor; every step appends to `workflow_transitions`
as a replay log; advancement is driven by `workflow:advance`,
`workflow:resume`, and `workflow:timeout-sweep` jobs on the existing
outbox. Restart-safety, retries, and deadline scheduling are inherited
from `core/jobs` — no second scheduler.

## 1. Concepts

Four tables (see `core/db/migrations/0016_workflow.sql`) are all you
need to reason about a run.

- **`workflow_defs`** — one row per slug per version. `spec_json`
...
The JSON body an admin POSTs. Source of truth is `core/workflow/spec.go`.
...
Six endpoints in `core/api/workflow.go`, mounted under
`/api/workflows/*`.

| Method | Path | Auth | Purpose |
|---|---|---|---|
| POST   | `/api/workflows`                        | admin      | Persist a new version of a spec for a slug. |
| GET    | `/api/workflows/{slug}`                 | authed     | Return the current active spec. |
| POST   | `/api/workflows/{slug}/start`           | authed     | Kick off a run against `doc_id`. |
| GET    | `/api/workflows/runs/{id}`              | authed     | Return `{run, transitions, tasks}` for one run. |
| POST   | `/api/workflows/tasks/{task_id}/resolve` | assignee or admin | Resolve a human task with a choice. |
| POST   | `/api/workflows/runs/{id}/cancel`       | admin      | Abandon a running run. |
...
| Code | HTTP | When |
|---|---|---|
| `workflow_disabled` | 503 | `SetDefault` was never called (engine not wired). |
...
```bash
curl -sS -X POST http://localhost:8000/api/workflows/invoice-approval/start \
  -H "Authorization: Token $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"doc_id": 17}'
```
...
The dispatcher is nudged so the first `workflow:advance` job fires
immediately. The `approve` handler spawns a `workflow_tasks` row
assigned to `user:5` and parks the run.

### Step 3: Inspect the run

```bash
curl -sS http://localhost:8000/api/workflows/runs/42 \
  -H "Authorization: Token $TOKEN"
```
...
```bash
curl -sS -X POST http://localhost:8000/api/workflows/tasks/101/resolve \
  -H "Authorization: Token $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"choice": "approve"}'
```

Returns `204`. The engine enqueues `workflow:advance` with
...
```bash
curl -sS -X POST http://localhost:8000/api/workflows/tasks/102/resolve \
  -H "Authorization: Token $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"choice": "approve"}'
```

`GET /api/workflows/runs/42` now shows `Status: "done"`,
...
`state.timeout_sec` sets `workflow_runs.deadline_at` on entry. Every
30 seconds (see `SweepInterval` in `core/workflow/sweeper.go`) the
`workflow:timeout-sweep` job scans:
...
For each due row it enqueues `workflow:advance` with
...
kind='workflow:timeout-sweep' AND state='pending'` so a double-invoke
...
type HandlerResult struct {
    ...
    Task  *TaskSpec       // non-nil spawns a workflow_tasks row and parks
}
```

- Handlers MUST be retry-safe — the outbox delivers at-least-once.
- Register at boot, before `workflow.SetDefault(engine)`.
...
```go
engine := workflow.New(dbHandle, logger)
...
- **Outbox visibility** — the three job kinds
  (`workflow:advance`, `workflow:resume`, `workflow:timeout-sweep`)
...
- **YAML definition loader.** Specs are JSON in
  `workflow_defs.spec_json`; a YAML compiler is additive and needs no
...
- **Merging `workflow_tasks` into `/api/tasks/`.** Today the outbox
  and `workflow_tasks` are surfaced separately; the mobile-compat
