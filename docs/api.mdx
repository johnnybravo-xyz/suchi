...
Most `/api/` responses are JSON; successful mutations may return an empty
...
carry it into bug reports. See [Errors](#errors) for the JSON error contract.
...
| Client compatibility     | `GET /api/handshake` (public; no credentials)                                                                                                                                         |
| Mobile pairing           | `POST/DELETE /api/mobile/pairing` (session), `POST /api/mobile/pairing/exchange` (one-time code)                                                                                         |
...
| Documents               | `POST /api/documents/`, `GET/PATCH/DELETE /api/documents/{id}`, `POST /api/documents/{id}/restore`, `GET/DELETE /api/trash/`, `DELETE /api/trash/{id}`                                      |
...
| Mail intake | `/api/email-accounts/*` (requires `mailboxes`; within the selected system, admins see all rows and members see their own) |
...
| Admin                   | `/api/admin/users`, `GET /api/admin/setup/state`, `POST /api/admin/setup/preset`, `/api/admin/settings/{llm,preferences,ingest}`, `/api/admin/taxonomy/{import,export}`          |

## Auth

The same authentication chain accepts browser sessions and API credentials:

- **Cookie** — set by local-password [JSON login](#post-apilogin), the
  `POST /login` form, or the OIDC callback. OIDC configuration disables local
  ...
  HTTPS.
- **API token** — `Authorization: Token <hex>` is preferred;
  `Authorization: Bearer <hex>` is also accepted. Suchi tokens are exactly
  64 lowercase hex characters. Other Bearer values go to the OIDC authenticator.

OIDC ID tokens must contain `email` and the boolean `email_verified: true`.
This applies to direct Bearer authentication and browser sign-in; see
[provider requirements](/config#oidc). Invalid credentials do not fall back to
an existing browser session.

API tokens are limited by an explicit route allowlist in
`distro/app/serve_token_policy.go`. Document reads (including search,
...
must explicitly declare their token access in the public assembly policy.
Compiled routes added through `app.Options.Configure` cannot extend that policy
and remain session-only.

## Filing-system request context

After [first prefixed import](/jd), collection requests select `?system=S01`.
...
See [Permissions](/permissions) for the shared administrator/host/backup trust boundary.
...
The sections below group routes by task. [Auth](#auth) and
[filing-system request context](#filing-system-request-context) apply throughout.

## Health and client compatibility
...
**200** `{"product":"suchi","api_version":1,"min_app_version":"0.1.0"}`
...
Pairing clients use the current exchange shape, including `device_name`.
...
When the bounded Argon2 worker is already at capacity, local-password login and
token exchange fail quickly with **503**, `Retry-After: 1`, and the same response
for known and unknown accounts. Password hashing for account and protected-share
creation uses the same bound and returns a retryable **503** when unavailable.
...
```json
{
  ...
  "build_version": "v0.1.0-beta.2",
  "build_revision": "1234567890ab",
  "role": "admin|member",
  "authn_by": "local-auth|oidc",
  "avatar_url": "/api/users/1/avatar?v=<sha256>",
  "system_id": 1,
  "system_name": "Archive",
  "system_code": "",
...
capabilities. Token responses also include the credential's filing-system
`system_id`, `system_name`, and `system_code`. These fields are omitted for
cookie/OIDC sessions. `system_code` remains present but may be empty before the
original archive is assigned a permanent code.
...
must first validate the scanned link against the [pairing origin rules](#post-apimobilepairing),
confirm the server with the user, and run the [compatibility handshake](#get-apihandshake)
...
- Disabling the caller's own account returns **403** `cannot_disable_self`,
  even when another active administrator exists.
- A role or active-status update that would leave no active administrator returns
  **409** `last_active_admin`. Disabled administrators do not count.

The active-administrator check and role/capability normalization use the current
row inside the write transaction. Local mailbox, share-link and shared-view
revocations use the same transaction. Capability audit writes also occur there:
successfully stored records follow committed transition order. Audit persistence
remains best-effort; failed audit writes are logged without rejecting an otherwise
valid update. Rejected patches and cascade failures leave the user and resources
unchanged; cascade failures return **500**. Later promotions or regrants cannot be
overtaken by older revocations. OAuth flows are invalidated only after a disable
commits; rejected disables preserve them. Resource logs, audit failure reports,
external audit delivery and runtime reloads happen after commit.

### Public-demo surface: `/api/demo/*`

Session creation and upgrade require `SUCHI_DEMO_MODE=1`
(see [demo-instance](/demo-instance)); otherwise they return
...
POST /api/demo/session/upgrade   → 200 {"kind":"scratch"} + HttpOnly session cookie
```
...
- `session` is unauthenticated — mints a signed anonymous token
  ...
  TTL defaults to 15 minutes. Browsers use the issued HttpOnly cookie;
  non-browser read clients can send the token in `X-Suchi-Demo-Token`.
- `session/upgrade` requires a valid anonymous token. Provisions a
  scratch user `visitor-<hex>@demo.local` and replaces the anonymous cookie
  with a digest-backed browser session. Its lifetime is the configured scratch
  TTL, not the normal 30-day login lifetime. It retains the constrained
  `demo-scratch` identity for both API calls and direct preview/download URLs.
  Upgrade clients must retain response cookies; no API token is returned.
...
The [target system](#filing-system-request-context) is captured before upload
...
children inherit their parent's system; [version uploads](#post-apidocumentsidversions)
...
| Field | Contract |
| --- | --- |
| `document` | Required file part. MIME is determined from stored bytes, not the multipart header. |
| `source_mtime` | Optional, single positive base-10 Unix-second integer. Duplicate, empty, zero, negative, signed, fractional, or malformed values return `400 bad_source_mtime`; omission stores `NULL`. |
| `content`, `content_source`, `content_confidence` | Optional device OCR group. All three must occur exactly once together; `content_source` must literally be `device_ocr`, content must be valid UTF-8 at most 1 MiB, and confidence must be finite in `[0,1]`. Invalid or partial groups return `400 bad_device_content`. |
| `ocr_language` | Optional only with the complete device OCR group. Accepts the existing `ll_RR` form or canonical BCP-47 `ll-RR`, stored as `ll_RR`; invalid values return `400 bad_device_content`. |
...
prevents a document duplicate; [version retries](#post-apidocumentsidversions)
...
- Hash matches a **trashed** document still inside its 30-day recovery window
  → clears `trashed_at`, returns `200 OK` with `"restored": true`. An expired
  match is not restored; the upload creates a fresh row while retention cleanup
  removes the expired one.
...
| `split_origin_id`                    | Positive upload document id; returns its live QR-split children after ACL filtering.                                              |
| `document_type__id`                  | Exact document type.                                                                                                        |
| `tags__id__in`                       | Up to 100 positive tag-ID tokens; duplicates are removed in first-seen order. Result must carry **every** unique ID (AND).   |
| `correspondents__id__in`             | Up to 100 positive correspondent-ID tokens; duplicates are removed in first-seen order. Result matches **any** (OR).         |
| `document_ids`                       | CSV of at most 100 positive document IDs; useful for exact snapshots. ACLs still apply.                                     |
...
| `lang`                               | Two- or three-letter language code. Input is trimmed and lowercased; exact stored language membership is required.           |
...
[filing-system request context](#filing-system-request-context) still applies.
...
```json
{
  ...
  "device_content_confidence": 0.86,
  "device_ocr_language": "en_US",
  "device_content_received_at": 1793494800,
...
Successful metadata PATCH and bulk metadata/tag edits remove only the
classifier-owned `needs-review` assignment in the same transaction. Manually
assigned, imported and rule-assigned review tags remain. Explicitly adding
`needs-review` takes human ownership before cleanup; Trash, restore and rescan
alone do not resolve a review marker.
...
bytes and use ETag `"documents.thumb_sha"`. Optional integer
...
return `400 bad_width`. Both forms use `Cache-Control: private, no-cache` and
support conditional requests with their ETag. Every reuse therefore revalidates
authorization, document state, and sensitivity before a matching request can
...
Each version node is checked against its current document ACL. A caller must be
able to view the requested node, and the response omits any other chain node it
cannot view. Relationship ids and head state are computed only from the visible
subset, so a later ACL change cannot leak hidden version metadata.

### `POST /api/documents/{id}/versions/`

Upload a new version with `previous_version_id={id}`. Uses the
[document upload multipart, device OCR, source timestamp and idempotency rules](#post-apidocuments).
...
- `restore` enforces the same 30-day window as individual restore. An expired
  authorized document rejects the batch with `400 bad_parameters`; already-live
...
- `remember` — optional. When `true`, the password is AEAD-sealed with the
  ...
  doc's owner and system. Future encrypted uploads by that owner in that system auto-try it.
...
- `404 not_found` — document missing, trashed, not encrypted, or unavailable to change
...
With `remember=true`, a successful batch stores the password once for the
...
on disk until [offline garbage collection](/cli#suchi-gc-flags), which reclaims
...
| `tags__id__in`, `correspondents__id__in`             | Up to 100 positive ID tokens per field, deduplicated in first-seen order; doc matches ANY.                                         |
| `document_type__id`, `jd_category_id`, `sensitivity` | Single-value filters.                                                                                                            |
| `lang`                                               | Two- or three-letter language token (`de`, `kn`, `hin`). Narrows to docs whose `languages` includes the token; input is trimmed and lowercased. Invalid values return `400 bad_lang`. |
...
sensitivity}`. `snippet` wraps a matching span in `<mark>...</mark>` for
...
DRF envelope. Read-only — tree changes use the admin preset/import endpoints,
not per-row insert. Generated 49 Inbox is included; 00–09 System index is not
an area/category row or picker destination.
...
Admin browser-session only. Returns `204 No Content` on success,
`404 not_found` otherwise. Document-tag junction rows are removed; surviving
direct children are detached from the deleted parent and become roots. The
handler explicitly detaches them within the deletion transaction, including
on databases with an already-applied schema-3 `ON DELETE CASCADE` parent FK.

### `DELETE /api/tags/`

Admin browser-session only. Body: `{"ids":[1,2]}` with 1–500 distinct positive
tag IDs. Returns `204 No Content` after one atomic selected-system transaction.
Every ID must exist in that system: a missing or foreign-system ID returns
`404 not_found` without deleting any tags or links. Malformed JSON returns
`400 bad_json`, an empty set `400 no_ids`, duplicate or nonpositive IDs
`400 bad_ids`, and more than 500 IDs `413 too_many_tags`. A non-admin receives
`403 forbidden`; API tokens cannot use this route even with write scope.
Document-tag links to deleted tags are removed. Any surviving children of
deleted parents become roots, including when both parent and child are selected.
...
ancestry does not imply document-search inheritance; see [Architecture](/architecture).
...
Admin-only preset catalog for Archive Configuration's filing-tree picker. Returns one row per
built-in preset defined in `core/jd/presets.go`; the picker renders
...
[
  {
    "id": "solo",
    "name": "Solo filing",
    "description": "One person's admin: ID, taxes, health, receipts. Compact 3-area tree.",
    "areas": [
      { "code": 10, "name": "Life admin", "category_count": 4 },
      { "code": 20, "name": "Money", "category_count": 4 },
      { "code": 30, "name": "Health", "category_count": 3 }
    ]
  },
  ...
]
...
`blank: true` marks the empty preset (no areas), which the picker
gates behind an explicit confirm checkbox. Categories aren't included
per row — they'd bloat the payload, and the picker doesn't render them.
Once a preset is applied via
`POST /api/admin/setup/preset`, the tree becomes visible through
`/api/jd/categories/`.

### `POST /api/admin/setup/preset`

Session-admin-only. Applies one built-in preset additively to the selected filing
system; API tokens cannot use it. Existing filing and locally edited rules are
preserved according to the same merge rules as taxonomy import.
...
`preset_id` is one of `solo`, `household`, `smb_billing`, `freelance`, or
`blank`. `confirm_blank` must be true for `blank`. `include_seeds` defaults to
true when omitted; false skips the preset's starter automations. `refile` runs
the existing explicit refile sweep after the preset is applied.
...
Unknown presets and unconfirmed Blank return `400`; unresolved merge collisions
return `409` without a partial application.
...
| Field | Meaning |
| --- | --- |
| `format` | Declared schema, `suchi-taxonomy/v1`, not serialization |
| `preset_id`, `preset_version`, `name`, `story` | Submitted author identity, content revision and explanation |
| `content_sha256` | SHA-256 of exact submitted bytes |
| `state_hash` | Binding of source hash, seed choice, remaps and relevant current configuration |
| `mode` | `replace` only for safe unconfigured bootstrap setup; otherwise additive `merge` |
| `system_code`, `system_name` | Proposed destination code/display name |
| `system_created`, `systems_introduced` | Whether Apply creates a destination or introduces the system layer |
| `existing_system_code` | Optional code preserving the original separately on first introduction |
| `user_areas` | Named incoming user `Area[]`, including category codes/names/descriptions |
| `generated_areas` | Separate `Area[]` containing 40 System / 49 Inbox only |
| `areas_incoming`, `categories_incoming` | Incoming user counts, excluding generated structure |
| `categories_to_add` | Actual category codes to add after remaps/skips |
| `collisions` | Objects `{code,existing,incoming,proposed_code,resolved}` |
| `keywords_to_seed`, `automations_to_seed` | Actual starter additions |
| `rules_to_add` | Rule names (`string[]`) |
| `rules_preserved` | Objects `{name,enabled}`, including existing disabled rows |
| `rules_skipped` | Objects `{name,reason}`, including category dependency skips |
| `applied` | Whether application committed |
| `index_refresh_pending` | Refresh is queued, not proof of a completed physical write |

The UI explains `00-09 System index` separately; it is not in either area array.
...
- `?limit=<n>` — 1–200, default 100. System and visibility predicates apply before
  cursor selection and pagination; a foreign event cannot consume the page or
  advance `latest_id`. Keep a separate cursor for each captured system.

**Visibility rules** (enforced server-side):

- Every returned row belongs to the selected system, including for administrators.
  Document rows also require current document visibility.
- Approval-task rows require system entry and any attached document's visibility,
  plus the exact `user:<id>` or `role:<role>` assignee unless the caller is an admin.
- Other non-document rows are visible only to their producing user/token or an
  administrator within that system. Truly global events such as `backup.written`
  are not part of this filing-system feed; use operator logs or an audit sink.
...
API tokens need `documents:read` to list tasks and `documents:write` for generic
approval resolution. Built-in inferred document changes additionally require a
live interactive browser session; a scoped token is not human-review proof.
...
Both return `404 not_found` unless the referenced row is dead and available within
...
the `vars` projection includes bounded proposed/current values, confidence,
reason, policy version, source freshness and authorized supporting documents.
Raw baselines and session proofs are not exposed. Every inferred field needs
review regardless of score; unknown actions remain read-only. Source/ACL or
human-baseline changes prevent application. Resolution records and queues a
decision, not a completed metadata write; the eventual effect rechecks it.
...
| GET    | `/api/acls/{kind}/{id}`                             | owner or admin                     |
| PUT    | `/api/acls/{kind}/{id}`                             | owner or admin                     |
| DELETE | `/api/acls/{kind}/{id}?principal_kind&principal_id` | owner or admin                     |
...
- `doc_ids`: 1–200 live documents from one enterable system. Members must own them;
  administrators bypass ownership, not an explicit request or token system ceiling.
...
If password hashing is temporarily unavailable, creation returns **503** with
`Retry-After: 1`; no share row is written.
...
Metadata, password responses, and downloads use `Cache-Control: no-store` so
revocation, expiry, or a password change cannot be bypassed by cached content.
...
List owned links in the selected system with `GET /api/share_links/`
...
Admin-only. `started_at` is the first administrator's creation time.
`current_preset` and `filing_tree_chosen` describe
the selected `?system=Snn`, not the archive's default system. The preset identifies
the last imported choice, not proof that the current merged tree equals that file.
An empty preset string and absent start time are omitted.

```json
{
  "started_at": 1789257600,
  "current_preset": "household",
...
Unicode-character [Research context preset](/archive-chat#research-context-presets)
...
| `precision` | Optional `day`, `month`, or `year`; requires `type=date` |
| `sort_from`, `sort_to` | Type-specific normalized range; dates use `YYYY-MM-DD` |
...
`accepted` is the storage status for a date available to Calendar. New inference
can reach it through guarded automatic application or interactive review. Older
accepted facts remain unchanged, including those with no recorded reviewer;
no reviewer is backfilled.
...
Rows contain generic `type`, `role`, `value`, `sort_value`, evidence, confidence,
review state, extractor/version and source-document presentation fields. Bounded
`reason`, `policy_version` and `source_current` explain review eligibility.
`evidence_start` records the matched raw date's UTF-8 byte offset, not a
case-folded string offset. Candidate IDs bind immutable proposals and are never
reused after rescan or deletion.
New automatic dates have `status=accepted`, null `reviewed_by`/`reviewed_at`,
`policy_version=threshold-auto-v1` and `reason=confidence_threshold`. Pending
candidates retain review-first eligibility reasons and policy. Historical
accepted/rejected rows keep their original provenance.

For `type=date`, `value` contains `date` and `precision`. Exactly three
precisions are supported:

| `precision` | Example `date` | Meaning |
| --- | --- | --- |
| `day` | `2026-08-06` | A specific day |
| `month` | `2026-08-01` | August 2026; day must be `01` |
| `year` | `2026-01-01` | The year 2026; month/day must be `01-01` |
...
probability. See [Document dates](/document-dates#date-precision-and-confidence)
...
call to the pipeline. New dates with usable source evidence follow the current
application mode and each date's own score; automatic mode defaults on and the
model/date threshold defaults to `0.70`. Other usable dates remain pending.
Extraction failure never permits automatic acceptance.
...
`decision` is `accepted` or `rejected`. Requires a live interactive browser
session, the intelligence capability, current system access and document
change permission. API/OIDC bearer possession is not sufficient.

Acceptance rechecks the current immutable source, extraction generation and
exact grounded evidence inside the write transaction. Legacy unbound pending
rows require fresh extraction. Authorized rejection can dismiss a stale or
unusable pending date without giving it authority. Results are granular:
`not_found`, `forbidden`, `stale_source`, `invalid_evidence`, or
`already_resolved`. A retry of the same recorded decision returns `ok: true`
without increasing `applied`; an opposite decision cannot rewrite history.
...
`auto_apply` is the shared mode for new dates, model metadata and local matching:
missing storage defaults to `true`; malformed policy fails closed. It does not
activate a model, grant egress permission or change explicit user-authored rules.

### `POST /api/admin/settings/llm`

Body: `{enabled, endpoint_url, model, api_key, clear_api_key, egress_ack,
confidence_threshold, archive_enabled, archive_review_threshold,
archive_auto_threshold?}`.
The model/date threshold must be between `0.50` and `0.95`, default `0.70`.
Each date is evaluated independently of the overall classification score.
The local automatic threshold defaults to `0.90` and must be `0.55` to `0.95`;
omitting `archive_auto_threshold` preserves its saved value. The local minimum
review-suggestion score defaults to `0.50`, must be `0.50` to `0.90`, and must
be strictly lower than the automatic threshold. Scores do not replace source,
access, field-generation or evidence checks.
`auto_apply` is PATCH-only; it and the old `date_auto_apply` are unsupported in
POST. Saving configuration never promotes pending dates or metadata or starts
an archive-wide backfill.
...
Updates exactly one standalone global setting. Choose either application mode:

```json
{ "auto_apply": false }
```

or the Archive research source-text preset:

```json
{ "research_context_mode": "detailed" }
```

The body must contain exactly one of these fields, not both or other fields.
`auto_apply` must be boolean; `research_context_mode` must be `focused`,
`balanced`, or `detailed`. The response returns only the saved field and value.

Application mode is read again under the writer before automatic effects; the
research preset applies to the next question. Neither standalone save rewrites
the endpoint, model, API key, egress acknowledgement or thresholds; enables or
tests a model; reloads the provider; accepts pending suggestions; or starts a
backfill. See [Archive research: Research context presets](/archive-chat#research-context-presets)
for passage assembly and exact payload bounds.
...
| `sealed_secret_b64` | Authenticated server-sealed actor/system-bound creation handoff from `/oauth/complete`, required for new Microsoft mailboxes. Never arbitrary at-rest MSAL ciphertext. |
...
and `files` rules do not accept matching criteria. The API accepts only the
`rules` shape; the former flat policy shape is not supported.
...
When `account_id` is omitted, Suchi returns an authenticated server-sealed creation
...
```json
{
  ...
  "username": "you@example.com",
  "oauth_account_id": "you@example.com",
  "sealed_secret_b64": "<base64 actor/system-bound creation handoff>"
}
```
