...
never replayed. The device token can later be revoked through session-only
token management. Pairing endpoints use `Cache-Control: no-store`, create no
browser session, and never log the code, link, QR, or token. Creation and exchange
share the login rate limit, including trailing-slash aliases.
