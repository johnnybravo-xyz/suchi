...
- **Token** — `Authorization: Token <hex>` header. Canonical scheme
  because it matches Paperless-ngx wire format so the mobile apps
  work unchanged.
...
`POST /api/workflows/tasks/{task_id}/resolve`. `workflow_open` in
`counts` is the true open-inbox size, unaffected by `limit`.

Paperless-compatible mobile clients that ignore unknown JSON fields see
the classic `counts` + `results` shape unchanged. suchi-native clients
read both arrays.
...
### Workflow surface: `/api/workflows/*`

State-machine engine for review chains and approvals. Six endpoints:

| Method | Path | Auth |
|---|---|---|
| POST   | `/api/workflows`                         | admin |
| GET    | `/api/workflows/{slug}`                  | authed |
| POST   | `/api/workflows/{slug}/start`            | authed |
| GET    | `/api/workflows/runs/{id}`               | authed |
| POST   | `/api/workflows/tasks/{task_id}/resolve` | assignee or admin |
| POST   | `/api/workflows/runs/{id}/cancel`        | admin |

See the [workflows guide](/workflows) for the spec shape, error codes,
timeout behavior, and a worked invoice-approval example.
...
Each `TagView` carries `parent_id` (nullable) and `child_count` so
the UI renders the shallow tree without extra round-trips.
...
The endpoints below are planned for Phase 4. Wire-compatible with
existing document-management mobile apps (`swift-paperless` on iOS,
`Paperless Mobile` on Android) so no bespoke app is needed.
