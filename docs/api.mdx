...
`languages` is a CSV of ISO-639-1 codes detected on the document
(via LLM classifier or metadata hints — see
[architecture § Languages](/architecture#languages)); empty when
nothing was detected yet. `languages_locked` is `true` once a user
