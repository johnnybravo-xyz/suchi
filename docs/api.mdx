...
`403 insufficient_scope`. API tokens cannot mint, list, or revoke tokens;
use a browser/OIDC session for token management. `/metrics` retains admin-token
support. New HTTP routes must explicitly declare their token access at the
assembly boundary.
...
system/category. Invalid canonical syntax or ID overflow is 400 `bad_address`;
...
canonical configured `PUBLIC_URL` origin, never a request or forwarded host.
...
must first validate the scanned link, confirm the server with the user, and
check `/api/handshake` without credentials or redirects. Production origins
require HTTPS; physical-device HTTP testing remains an explicit local-development
exception.
...
buckets across canonical, trailing-slash and percent-encoded slash aliases.
...
Admin creation (`POST /api/admin/users`) and updates canonicalize stored
...
The target is captured before upload/deduplication. Equal bytes uploaded by one
owner to different systems create distinct document rows but may share a CAS hash.
Idempotency keys remain user-global with system-bound fingerprints: cross-system
reuse conflicts. Replays recheck current membership/token authority. Pre-upgrade
stored receipts may omit address fields; numeric lookup returns the current address.
New versions inherit the predecessor's system/owner/category and reject a conflicting
request target. Split and email-attachment children inherit their parent's system.
...
canonical numeric confidence, and canonical OCR language using length-prefixed
...
canonical `Location`, and add `"idempotent_replay":true` without adding a source
or job. Reusing a key with different normalized input returns
`409 idempotency_conflict`. Scope and object authorization run on every request,
...
prevents a document duplicate; a version retry with the same predecessor and
bytes is recovered as a successful replay.
...
- Hash matches an existing **live** doc → records the upload as another
  ...
  Dedup remains system-and-owner-scoped.
...
All successful response shapes may include `"idempotent_replay": true` on a
matching keyed retry.
...
the explicit/token system boundary. Neither response returns the stored provider error.
...
system. Names, metadata references and rule forks are scoped there. Full guide at
...
List of docs awaiting a password in the selected system; additionally owner-scoped for non-admins.
...
Per-document custom-field values can currently be written and deleted, but are
not returned by document detail or a dedicated read endpoint. `/api/custom_fields/`
lists field definitions, not document values. Authenticated document-link writes
require permission to change the source and view the target in their respective
filing systems. A future value read endpoint must recheck target visibility and
redact inaccessible links; read-time redaction is not an implemented capability.

Write a typed custom-field value. `{field}` accepts either the numeric
field id OR the field name.
Definitions resolve within the source document's system. Ordinary metadata values
cannot reference another system, even when the actor enters both. A `documentlink`
may cross systems only when the authenticated actor can change the source and view
the live target, including entry to both, checked in the source write transaction.
Reads and link-related audit projections hide target IDs/labels that are no longer
viewable. Actorless handler/automation writes accept only same-system live targets.
...
and JD category (a filed doc's version stays filed). The new version keeps the
predecessor's immutable system, owner, sensitivity and explicit document ACL grants. A conflicting
requested system fails; version upload is not a document transfer.

The device OCR, `source_mtime`, and `Idempotency-Key` rules are identical to a
fresh document upload. A matching keyed response-loss retry replays before live
blob detection. Once the 30-day response cache expires, a keyed retry of the
same bytes against the same predecessor is still recovered from the resulting
version row. Without a replay, bytes already owned by any live document for the
predecessor's owner in that same system return
...
archive_auto_threshold, archive_review_threshold}`. The model threshold must be
...
The body must contain exactly `research_context_mode`, with a value of
...
This standalone save applies to the next question. It does not rewrite the
endpoint, model, API key, egress acknowledgement, confidence thresholds, or
date settings; enable a model; auto-apply pending dates; or reload the provider.
See [Archive research: Research context presets](/archive-chat#research-context-presets)
