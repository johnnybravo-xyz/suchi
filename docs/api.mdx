...
| Auth (JSON) | `POST /api/login`, `POST /api/token/`, `GET /api/whoami` |
...
| Taxonomy | `/api/correspondents/`, `/api/document_types/`, `/api/storage_paths/`, `/api/tags/`, `/api/tags/{id}/parent`, `/api/custom_fields/` |
| Classifier rules | `/api/rules/` CRUD |
| Search + autocomplete | `/api/search/`, `/api/autocomplete/` |
| Saved views + UI prefs | `/api/saved_views/`, `/api/ui_settings/` |
