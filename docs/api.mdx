...
global taxonomy mutations, profile management, raw-original downloads (`raw=1`), and profiling require a
...
`{"name":"My phone"}` names the resulting device token; omission or blank uses
...
random bits; storage contains only its SHA-256 digest, owner, name, and expiry.
...
Public exchange accepts JSON `{"code":"<64 lowercase hex chars>"}`. The app
...
response. The code is atomically consumed for an enabled user before token
issuance; concurrent scans cannot issue two tokens. Invalid, expired, replaced,
used, and disabled-account codes all return **400** `pairing_invalid`. If token
issuance fails or its response is lost, start a new pairing: consumed codes are
never replayed. Successful issuance stores `source: "mobile_pairing"` with
the token in the same transaction; generating or failing to exchange a code
does not register a connected app. The exchange response is unchanged.

Session-only `GET /api/tokens/` returns active credentials with `id`, `user_id`,
`name`, `scopes`, `created_at`, optional `last_used_at`, and optional `source`.
Paired credentials have `source: "mobile_pairing"`; manual and older credentials
omit it. Members see their own credentials; administrators retain their existing
all-account view. The list uses `Cache-Control: no-store` and contains no token
or token hash. `DELETE /api/tokens/{id}` revokes a device; logout-revoked tokens
...
fields. Matching retries replay the original `200` or `201`, preserve the
...
is the visible extracted-fact review count for callers with the
`archive_intelligence` capability. `dead_jobs`
...
Returns a paginated envelope of trashed documents. Members see their own
documents; administrators see all trashed documents. Each row includes
`trashed_at` and `deletes_at`, the Unix timestamp exactly 30 days later.
...
Permanently deletes every document in the caller's Trash scope for which the
caller has delete permission. Members are owner-scoped; administrators are
archive-wide. Returns `200 OK` with `{"purged": N}`.
...
document is trashed does not reappear. Documentless tasks remain visible while
their run still has actionable work. Resolve one via
...
predecessor's owner and explicit document ACL grants.
