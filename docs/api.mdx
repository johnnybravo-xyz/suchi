...
| `q`                                  | Rich query string shared with ranked search; plain terms use prefix `AND`.                                                   |
...
can navigate to `#/documents?jd_category_id=49` without name-matching. `dead_jobs`
...
**Ranking — BM25F + recency decay.** The ORDER BY is:

```
bm25(documents_fts, 3.0, 1.0)
  - 0.5 / (1.0 + (now - created_at) / 30_days)
```

- `bm25(documents_fts, 3.0, 1.0)` is BM25F with per-column weights
  passed to SQLite FTS5's built-in `bm25()`. Title matches carry
  3× the weight of content matches — a doc titled "Invoice 2026"
  outranks a doc that merely mentions "invoice" deep in body text.
- The subtracted term is a hyperbolic recency boost with a 30-day
  half-life. New docs get up to `-0.5` off their raw BM25F score;
  30-day-old docs get `-0.25`; year-old docs get `-0.04`. The boost
  is modest by design — it nudges genuinely-close matches without
  ever flipping a much-more-relevant older doc below a marginal
  newer one. Set `?recency=off` for raw BM25F ordering.
- Hyperbolic decay `1/(1 + age/half_life)` is used instead of true
  `exp()` because modernc/sqlite doesn't always link SQLite's math
  extensions. The two curves agree closely at the range we care
  about (0–2 half-lives).
...
`GET /api/search/` and `GET /api/documents/` compile `q` through the
same parser and return the same matching document set. Plain words are
Unicode-aware prefix terms joined with implicit `AND`; quoted text is a
phrase. Prefix a term or filter with `-` to exclude it.

```text
"distribution advice" jd:22 from:"Bagmane Prime" -tag:archived added:>=2026-01-01
```

| Token | Effect |
| --- | --- |
| `jd:22`, `jd:"22 Investments"` | Filing category by visible code or category name, never row ID. |
| `tag:tax` | Exact tag name. |
| `from:"HDFC Bank"` | Primary or linked correspondent. |
| `type:statement` | Exact document type name. |
| `sensitivity:confidential` | `public`, `internal`, `confidential`, or `restricted`. |
| `lang:de` | Assigned or detected 2–3 letter language code. |
| `title:invoice`, `content:"distribution advice"` | Restrict a text term to one indexed column. |
| `added:2026-08-25`, `added:>=2026-01-01` | Added date in UTC; `=`, `>`, `>=`, `<`, and `<=` are supported. |
| `date:2026-09-14`, `date:>=2026-09-01` | Human-accepted date intelligence; the same comparison operators are supported. |
| `date-role:renewal` | Accepted date intelligence with one semantic role. |
| `is:inbox`, `is:trash`, `is:encrypted`, `is:dated` | Current document state or accepted-intelligence presence. |

The parser rejects unknown filters, raw FTS5 expressions, Boolean `OR`,
grouping, proximity operators, malformed comparisons, unterminated quotes,
and non-whitespace control characters. Queries are limited to 1 KiB, 64
tokens, and 256 bytes per value. `400 bad_query` responses include `position`,
optional `filter`, and optional `suggestions` fields. Positions are byte
offsets in the query exactly as received, including leading whitespace.

`GET /api/autocomplete/?q=...` recognizes an incomplete qualifier at
the end of `q`. Suggestions for filter names and values carry `query`,
the full replacement query string. JD suggestions display visible code
plus name and insert the visible code. Qualifier completion accepts the same
1 KiB input limit as the parser. The older plain taxonomy-prefix mode remains
...
`MATCH` query, ranks by **BM25F (same title 3× / content 1×
weights as `/api/search/`)**, and returns the top N. Scope:
`documents:read`. Visibility-scoped identically to the list surface;
the source doc is always excluded from its own results. No recency
term here — "similar" is about content, not freshness.
...
`matched_on_title_only` is `true` when the source doc's extracted
content is empty and the pipeline could only match on the title. The
SPA renders this as a warning pill on the results card because the
match quality drops sharply without body vocabulary. The endpoint
also drops rows whose BM25 score falls below the noise floor —
see [similar documents](/similar-docs) for the full ranking story.

`method` reports the ranking backend (`"fts"`). Semantic embedding retrieval is
not shipped in v0.1.
...
History contains at most two complete user/assistant pairs and 12 KB.
`context_source_ids` contains at most six positive IDs. Scope accepts a rich
query, up to 100 exact IDs, and an optional JD category. Every source and
context ID is reauthorized.

The response contains at most six sources, validated citations, grounding
state, and intelligence counts:
...
Insufficient evidence returns a stable insufficient answer. Malformed or
out-of-range citations return `502 invalid_provider_response`; provider
failures return `502 provider_failure`; a disabled runtime returns
