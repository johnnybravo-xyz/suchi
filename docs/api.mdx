...
Admin-only. `started_at` is the first administrator's creation time and supports
the fresh-install reminder. `current_preset` and `filing_tree_chosen` describe
