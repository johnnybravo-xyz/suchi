...
Account/settings administration, credential vaults, global taxonomy mutations,
profile management, raw-original downloads (`raw=1`), and profiling require a
...
`403 insufficient_scope`. Token minting retains its subset-only rule for
existing-token callers; `/metrics` retains admin-token support. New HTTP
routes must explicitly declare their token access at the assembly boundary.
...
record. Rendered files are removed safely. CAS blobs are deleted only after a
post-commit reference check confirms no document or avatar still uses them.
