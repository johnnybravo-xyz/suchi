...
description: "Document questions with citations, exact source views, and reviewed extracted facts."
---
...
Every research scope below is bounded by the captured current filing system plus
membership/document permissions. Candidate retrieval, prior-turn IDs, accepted
facts, pending suggestions and context passages use that boundary; a multi-system
member cannot ask one request across both. Switching systems destroys browser
conversation state and rejects late responses. Shared model configuration and
FTS/CAS infrastructure do not make foreign documents eligible evidence.
...
validation rules, model-score signals, automatic or reviewed dates, legacy facts, rescans,
...
**Open N calendar dates** counts accepted date facts across all retrieved
documents, including sources the answer did not cite. It opens their all-dates
agenda, not just the current month. See
[Document dates: Research links](/document-dates#dates-linked-from-archive-research)
for scope, filters, and pagination.
...
Public demo sessions are denied both capabilities. A narrow Calendar read
exception exposes accepted, curated `demo-corpus` dates from visible documents;
it grants no access to model output, extraction, review, or Archive research.
...
The document-scoped Calendar agenda uses this same list endpoint with
`status=accepted`, `type=date`, `document_ids`, `page`, and `page_size=500`, without
`sort_from` or `sort_to`. Results remain in chronological order across pages;
the optional `role` filter still applies.
