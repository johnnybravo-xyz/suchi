...
description: "Document questions with citations, exact source views, and human-reviewed extracted facts."
---
...
- Approved facts are limited to three per source and twelve total; the response
  summary still counts every visible approved and pending fact.
...
The Approvals screen groups extracted facts by source document and supports
selection at candidate, document, and visible-result levels. Accept or reject
is one bounded, audited transaction with per-document authorization.

The Calendar reads only extracted dates a person approved. It supports month
and agenda views, date-role filtering, and Saved View scope. Pending or rejected
dates never appear.

Facts a person approved may be included beside their original source snippet in
a later archive answer. The original document remains the evidence and is still
cited.
...
`archive_intelligence`, also implicit for administrators. Approved dates are
...
Retrieval, source snapshots, extracted fact lists, approved facts in chat, and
...
`intelligence` summary. Approved facts attached to a source appear under that
...
4. Ask a question whose answer uses the approved date.
