...
- Extracted facts are limited to three per source and twelve total; the response
  summary still counts every visible fact by status.
...
For the end-user workflow, settings, review screen, and troubleshooting, see
...
When **Add high-confidence dates to Calendar automatically** is enabled, a date
at or above the user-defined model confidence threshold goes straight to
Calendar. Lower-confidence dates wait in Approvals. Enabling the option or
lowering the threshold immediately moves matching pending dates into Calendar
without another model call. Turning the option off sends newly extracted dates
to Approvals; it does not remove dates already in Calendar. Rescan only when a
document needs date extraction run again.

### Review and calendar

The Approvals screen groups dates still needing a decision by source document.
Selection works per date, source document, or visible result set; one bounded,
audited transaction adds or rejects the selected dates after rechecking
document authorization.

Calendar reads dates that were added automatically or approved in Approvals. It
supports month and agenda views, date-role filtering, and Saved View scope.
Pending or rejected dates never appear.
