...
option (iCloud, Yahoo, Zoho, Dovecot, etc.).
...
That's fewer components and enough for most operators.
...
| `suchi-data/`  | Suchi's `suchi.db`, blob CAS, backups.                                                     |
...
<Warning>
  Mounting `docker.sock` grants suchi root-equivalent access to the host Docker
  daemon. Suchi only uses it to restart the sibling mbsync container, but the
  socket permits arbitrary daemon calls. Comment out the mount + unset
  `MAIL_SETUP_DOCKER_SOCK` if you'd rather `docker compose restart mbsync`
  yourself after a wizard save.
</Warning>
