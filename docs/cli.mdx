...
The CLI is the surgical path — you pick filters, run the command,
own the outcome. The Tasks inbox is the **discovery** path for the
same feature: whenever `pipeline_version_ocr`, `pipeline_version_llm`,
or `pipeline_version_content` in the binary is newer than what any
docs are stamped with, boot-time detection opens a `rescan-proposal`
approval card in the inbox. Admins see it as *"Pipeline ocr · 520
documents stale (v1 → v2)"* with three choices — **approve all**,
**approve sample** (20 docs), or **dismiss**. Approvals route
through the same `core/rescan.Enqueue` helper the CLI uses, so the
two surfaces are always in lockstep.
...
boot. See [Approvals engine → built-in defs](/approvals#built-ins)
for the state-machine detail.
