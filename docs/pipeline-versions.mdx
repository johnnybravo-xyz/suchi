...
The Tasks card reports the pipeline, target revision, and matching document
count. Its actions are:
...
Exhausted or terminal jobs appear under **Dead jobs** in Tasks. Correct the
underlying problem, then choose **Retry** to reset the same durable row and run
it again. Choose **Dismiss** when the failure was expected or no longer needs
attention. Both actions are administrator-only and audited; dismissing a job
does not delete its document.
