...
The proposal in **Approvals** reports the pipeline, target revision, and
matching document count. Its actions are:
...
Beta.2's raster/HEIC encoder correction keeps content revision `2` and OCR
revision `1`. It does not propose an archive-wide rescan for this image-only
bug fix. Original images remain available even when an earlier installation
stored invalid derived bytes or skipped conversion. Select affected images
in Documents and choose **Rescan** to regenerate their archive and indexed
text. Upgrading alone does not repair previously processed images.
