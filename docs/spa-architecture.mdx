...
Profile saves send only the supported `display_name`; email is a read-only sign-in
identifier. My account uses `whoami.kind` to keep mobile pairing out of both demo
session kinds, matching the server's pairing eligibility.
...
helpers capture the same destination. Text taxonomy exports use the shared request
helper, including its logout and generation guards and structured API errors.
A `system_unavailable` response for the current system clears the active surface
rather than presenting another cabinet; background upload or pairing-cleanup errors
for an earlier destination cannot invalidate the current one.
Every new upload entry point, including window file drops,
requires the current route's system to be ready. These browser guards
...
The initial public `/api/demo/mode` request survives filing-context initialization;
it still obeys session revision cancellation. This lets first-time demo visits mint
their cookie and open the tour before normal collection loading.
...
remain unchanged.
The signed-in user's Active switch reads `session.user.user_id`; the API remains
authoritative for self-disable, last-active-administrator, and capability
