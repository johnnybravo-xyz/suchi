...
The beta build keeps the shell, archive workflows, management workflows, and
the on-demand archive-question drawer separate. Keep the shell at or below 35
KB gzip and remeasure every chunk after UI changes. The current production
chunks, measured with system `gzip`, are 33,513 bytes for the shell, 24,204 for
archive workflows, 42,113 for management workflows, 5,162 for lazy Archive
research, 384 for shared intelligence helpers, and 13,732 for CSS. This is a
transfer-size budget, not a browser-memory or container-size claim.
