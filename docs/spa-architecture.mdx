...
`TaxonomyImport.svelte` is shared by setup and Settings. It owns file/paste reads,
...
`lib/configuration.js` owns one section inventory; setup derives its order/labels
with the shorter People label. `Setup.svelte` owns navigation/completion;
...
gates and writes across setup/Settings. Mailbox editing delegates to
...
`UserCreateForm.svelte` shares creation/validation between setup and Settings;
it submits no hidden member capabilities for admins. Self-disable checks use
...
`cd ui && bun run e2e --grep 'separates completed archive administration|keeps archive layout|refreshes intake owners|failed configuration reads'`.
...
| Setup/settings sections | `lib/configuration.js` |
...
These screens reuse existing tasks, intelligence and document APIs; no trust-status
or document-history endpoint is added.
