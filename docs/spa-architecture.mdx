...
in component memory, clears expired QR/link displays, and cancels an unused
code on close or route navigation. A session refresh or account replacement
clears and closes the prompt; generation and clipboard writes check its original
owner before starting. Late responses cannot populate another account's prompt.
...
session; account changes clear the list, and unmount stops polling. Revocation
reloads the list so an earlier poll cannot restore a disconnected app.
...
| --- | ---: | ---: |
| Shell: navigation, Dashboard, login | 95,761 | 34,729 |
| Documents / Inbox | 27,360 | 10,046 |
| Document detail | 35,114 | 12,293 |
| Upload | 13,111 | 5,953 |
| Search | 8,894 | 4,304 |
| Views | 14,630 | 5,275 |
| Calendar | 16,336 | 6,411 |
| Trash | 8,327 | 3,913 |
| Approvals | 18,351 | 6,594 |
| Automations | 17,942 | 5,936 |
| My account, without mailbox capability | 15,911 | 5,797 |
| Mobile pairing prompt, when opened from My account | 8,008 | 3,941 |
| Archive configuration, including its Settings entry | 110,488 | 36,622 |
| Setup | 66,510 | 22,161 |
| Demo | 5,583 | 2,340 |
| Archive research drawer | 13,763 | 5,296 |
| **All unique JavaScript chunks** | **395,997** | **141,286** |

The initial shell also transfers 5,092 gzip bytes of CSS. All CSS together is
22,437 gzip bytes. The complete embedded SPA tree is 489,409 raw bytes. Shared
chunks are cached across navigation; do not sum the route rows to estimate the
complete build. A mailbox-capable member additionally loads 10,464 gzip bytes
...
unique JavaScript chunks from 26 to 22 and Documents' additional requests from
