...
| Chunk | Screens or purpose | JavaScript bytes | CSS bytes |
| --- | --- | ---: | ---: |
| Shell | Navigation, Dashboard, and login | 34,306 | 4,944 |
| Document work | Documents, document detail, and Upload | 16,578 | 0 |
| Search | Search | 2,481 | 0 |
| Organize | Views, Calendar, and Trash | 7,686 | 2,759 |
| Workflow | Approvals and Automations | 11,125 | 1,953 |
| Configuration | Settings and Setup | 32,457 | 4,416 |
| Demo | Demo workspace | 2,360 | 1,445 |
| Archive research | On-demand research drawer | 5,269 | 2,352 |
| Query helpers | Shared by Documents and Search | 467 | 0 |
| Date helpers | Shared by Calendar and Approvals | 500 | 0 |
| **All chunks** | Complete production asset set | **113,229** | **17,869** |

The initial application transfer is the 34,306-byte shell plus 4,944 bytes of
shell CSS. Route CSS loads with its lazy JavaScript chunk; a route transfer also
includes its small shared helper where listed. These are transfer-size
measurements, not browser-memory or container-size claims. The complete
embedded SPA tree is 426,030 bytes before HTTP compression, but ordinary
navigation fetches only the shell, the selected route group, and their listed
dependencies.
...
Ordinary browser login uses a session cookie; scripts do not read its value.
The public demo keeps its anonymous read token in `sessionStorage` and its
upgraded scratch-user API token in `localStorage`. The API helper supports that
token alongside cookie sessions. Cookie-authenticated mutations remain
same-origin and pass Fetch Metadata protection on the server.
