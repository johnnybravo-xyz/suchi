...
| `date:` | Approved extracted date using `YYYY-MM-DD`. Supports the same comparisons. Pending and rejected dates never match. |
| `date-role:` | Role of an approved date: `issued`, `due`, `start`, `end`, `expiry`, `renewal`, `service`, or `other`. |
| `is:` | `inbox`, `trash`, `encrypted`, or `dated`. `is:dated` requires an approved extracted date. |
...
approved date fact. Negated date clauses remain independent `NOT EXISTS`
conditions. Approved dates are readable document metadata, so these predicates
