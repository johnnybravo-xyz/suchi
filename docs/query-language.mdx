...
| `date:` | Human-accepted date intelligence using `YYYY-MM-DD`. Supports the same comparisons. Pending and rejected dates never match. |
| `date-role:` | Accepted date role: `issued`, `due`, `start`, `end`, `expiry`, `renewal`, `service`, or `other`. |
| `is:` | `inbox`, `trash`, `encrypted`, or `dated`. `is:dated` requires accepted intelligence. |
...
accepted date fact. Negated date clauses remain independent `NOT EXISTS`
conditions. Accepted dates are readable document metadata, so these predicates
require `documents:read`, not `archive_intelligence`.
...
correspondent, type, sensitivity, and accepted-date controls are converted into
...
deduplicated IDs. They preserve one exact source set while still reapplying ACLs
whenever opened or shared.

## Archive research and generated Views
...
could differ from the cited sources.

**Save source set** therefore creates an exact snapshot from authorized source
IDs. A live View that should match future documents is created explicitly with
this query language.

- RAG retrieval optimizes recall for one answer.
- Rich queries define stable live filters.
- Snapshot Views preserve exact cited evidence.
