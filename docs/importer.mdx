---
title: "Importing from Bundle"
description: "suchi import bundle — verbatim metadata, dedup, --map-jd, --auto-jd, --verify."
---

`suchi import bundle` consumes a standard `document_exporter`
...
- A completed an existing DMS export at `/path/to/export`. If you don't
  have one, run this on the Bundle side:
  ```bash
...
```bash
suchi import bundle --from /path/to/export --owner-email you@example.com
```
...
3. For each document, streams the original (and optional archive)
   ...
   transaction. `bundle_id_legacy` is preserved for round-tripping
   and dedup.

## Dedup

Documents are keyed by the SHA-256 of the ingested bytes and by
`bundle_id_legacy`. A re-run against the same bundle skips every
...
<Tabs>
  <Tab title="--flat">
    ...
    you're a Bundle-classic migrator and want your tag/correspondent
    workflow untouched — the inbox is your one bucket, JD affordances
    stay hidden in the UI.

    ```bash
    suchi import bundle --from ./export --owner-email you@ex.com --flat
    ```
  </Tab>
  <Tab title="--map-jd">
    ...
    ```bash
    suchi import bundle --from ./export --owner-email you@ex.com \
        --map-jd mapping.yaml
    ```
  ...
  <Tab title="--auto-jd">
    ...
    ```bash
    suchi import bundle --from ./export --owner-email you@ex.com --auto-jd
    ```
...
```bash
suchi import bundle --from ./nightly-export --verify
```

Report partitions every document in the bundle into:

- **New** — would be imported (no matching `bundle_id_legacy`).
...
- **Orphan** — present in suchi with a `bundle_id_legacy` that
  isn't in this bundle. Suggests a Bundle-side deletion since the
  last export.

Compared fields (Phase 1): `title`, `original_size`. Deliberately
minimal — OCR text drifts trivially across Bundle versions and
...
<Steps>
  <Step title="Stand suchi up alongside Bundle">
    Point it at the same backup pipeline.
  </Step>
  <Step title="Every night, verify">
    Run `document_exporter` on the Bundle side and
    `suchi import bundle --verify` on the suchi side.
  </Step>
  ...
  <Step title="Cool off">
    Keep Bundle running for a two-week cool-off, then decommission.
  </Step>
</Steps>
