...
- the JD category changes only while the document remains in the inbox; and
- an extracted date goes directly to Calendar when **Add high-confidence dates
  to Calendar automatically** is enabled.

See [Document dates](/document-dates) for the automatic-versus-reviewed
Calendar workflow.
...
the document owner. Extracted dates wait in Approvals. Turning automatic
Calendar dates off sends every newly extracted date to Approvals.
...
applies immediately. No Suchi restart is required. Enabling automatic dates or
lowering the threshold promotes matching pending dates without another model
call. Raising the threshold or disabling automatic dates never removes dates
already in Calendar.
