...
Suchi ships deterministic filing rules and archive-based heuristics. The
...
state. Hosted mode also requires an explicit egress acknowledgement.
...
A changed non-empty title becomes a proposal. The built-in
`apply_llm_title` automation applies it only above that automation's threshold;
otherwise it remains in Tasks for review.

Below the classifier threshold, source fields stay unchanged, Suchi adds the
`needs-review` tag, and archive-based filing may offer a corroborating proposal.
...
Saving enabled state, endpoint, model, key, timeout, acknowledgement, or
confidence applies to the running process immediately. No Suchi restart is
required.
...
- **Low confidence:** use a stronger model, improve deterministic filing rules,
  or keep the conservative threshold and review proposals.
