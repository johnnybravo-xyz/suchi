...
For **Archive research**, it instead sends the question, up to four preceding
messages, and at most six short snippets selected from documents the caller may
read. Reauthorized context and human-accepted intelligence may accompany those
same sources. Sensitive snippets require explicit per-conversation consent.
...
archive research, exact source-set Views, generic human-reviewed intelligence,
and accepted-date Calendar/query workflows. It does not provide arbitrary
model tools, custom code in the ingest pipeline, persistent chat, timed calendar
events, or implicit rule learning from approvals.
