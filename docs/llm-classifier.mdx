...
description: "Optional local or hosted model assistance, source-grounded research, and threshold-based suggestions with optional review."
---
...
state. The same screen controls local archive matching and its automatic/review
thresholds. **Automatically apply high-confidence suggestions** is on by default;
turn it off for review-first behavior. **Save application mode** saves that
choice independently. Hosted mode still requires an explicit egress
acknowledgement; the mode does not activate a model or grant egress permission.
...
With automatic mode on, eligible model metadata at or above the threshold can
apply without review. With it off, valid inferred changes go to Approvals even
at confidence 1.0. Below-threshold changes remain review suggestions. Scores are
model-reported signals, not calibrated probabilities of correctness.
An unset correspondent and Inbox-only filing remain eligibility checks. Human
edits, clears and language locks take precedence over in-flight inference;
automatic language changes do not create a human language lock.

Local archive matching shares the same mode, with a default automatic threshold
of `0.90` and minimum review-suggestion score of `0.50`. Explicit user-authored
rules remain independent.
Existing custom thresholds are preserved. An older review-only configuration
with a `0.90` review floor and no saved automatic threshold uses `0.95` for
automatic application, keeping its review floor valid.

Date extraction shares the request but requires exact target-document evidence
and evaluates each date's own score against the model/date threshold, independently
of overall classification confidence. See [Document dates](/document-dates).

Similar-document titles used as naming context are selected under the document
owner's access, not an administrator's archive-wide bypass. Their identities,
source snapshots and visibility are rechecked before results are stored and
again when a metadata suggestion is applied.
...
An authorized human metadata PATCH or bulk metadata/tag edit resolves a
classifier-owned `needs-review` assignment in the same write transaction.
Manual, imported, rule-owned and legacy review tags remain; explicitly adding
`needs-review` takes human ownership before cleanup rather than removing it.
Trash, restore and rescan alone do not count as review.
...
key, timeout, acknowledgement or confidence threshold applies immediately.
No Suchi restart is required. A provider-configuration change during a model
call invalidates that result; the current application mode is checked again
inside the writer before automatic changes. Saving settings never accepts
pending suggestions.
...
prompt revision can bump that version. Maintainers separately raise the tagged
LLM recommendation when existing successful results should receive a
stale-results proposal. Changing endpoint, model, key, timeout, or threshold
does not mark the archive stale.
...
The same classification call may propose typed facts. Dates are the first
registered type, stored in the generic `document_intelligence` ledger. Only
accepted dates reach Calendar, rich-query filters or later answers: acceptance
may be automatic under the current mode and threshold, or explicitly reviewed.
Automatic acceptance records no human reviewer and does not enable ICS export.
...
archive research, Views made from retrieved documents, source-backed facts,
and document-date Calendar/query workflows. It does not
