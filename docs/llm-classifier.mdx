...
Stored web settings take precedence over environment fallbacks, following the
canonical [configuration precedence](/config#precedence). Prefer one management
path per deployment so partial overrides remain easy to reason about.
