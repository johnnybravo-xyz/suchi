...
The compose recipe wires suchi's admin-only `/admin/mail-setup` page
to the same `config/.env` file. Log in as an admin, open the wizard,
pick a provider, save — suchi rewrites the file and restarts mbsync
in place, no shell needed.

The wizard is opt-in via two mounts already set in `docker-compose.yml`:
