...
mail-intake emits Bundle-native flat JSON:
...
`core/ingest/sidecar` transparently accepts the Bundle-native
shape — no producer patch needed. `"Name <address>"` correspondent
strings are peeled to just the display name; `created` accepts
RFC-3339 or `YYYY-MM-DD`.

If you're writing your own producer, you can EITHER emit the
Bundle-native shape (portable) OR the versioned suchi-native form
(access to `jd_category`, `custom_fields`, multi-role correspondents,
etc. — see [sidecar spec](/architecture#json-sidecar)).
