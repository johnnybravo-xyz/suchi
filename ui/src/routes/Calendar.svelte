<script>
  ...
  function dateOriginLabel(event) {
    ...
    if (event.reviewed_at) return 'Reviewed'
    if (event.status === 'accepted' && !event.reviewed_by && event.policy_version === 'threshold-auto-v1') return 'Automatic'
...
<div class="calendar-page">
  <header class="calendar-intro">
    <div>
      ...
      <h2>Calendar</h2>
      <p>{demo ? 'Read-only examples from the demo documents. Dates are curated from the sources, not generated by a live model.' : 'Automatic, reviewed, and previously accepted dates appear here. Dates awaiting review remain in Approvals. Each date links to its source document.'}</p>
    </div>
  ...
  <div class="calendar-layout" class:agenda-only={agendaOnly}>
    ...
    <aside class="agenda" aria-labelledby="agenda-title">
      ...
      {:else if !events.length}
        <div class="agenda-empty"><Icon name="calendar" size={34} /><b>{allDates ? 'No matching dates' : selectedDay ? 'No dates this day' : 'No dates this month'}</b><span>{allDates ? 'Check the date role, or review proposed dates in Approvals.' : 'Review proposed dates in Approvals, or choose another document view.'}</span></div>
      {:else}
          ...
          {#each events as event (event.id)}
            ...
            <article class="agenda-event">
              ...
              <div class="agenda-copy">
                ...
                {#if event.evidence_text}<span>“{event.evidence_text}”</span>{/if}
