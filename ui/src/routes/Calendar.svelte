<script>
  ...
  const allDates = $derived(Boolean(documentIDs))
  ...
  function calendarHref({ date = selectedDay, monthValue = month, viewID = selectedViewID, dateRole = role } = {}) {
    ...
    if (allDates) params.set('document_ids', documentIDs)
    else {
...
<div class="calendar-page">
  ...
  {#if allDates}
    <div class="calendar-scope">
      <p>Dates from {documentCount} selected document{documentCount === 1 ? '' : 's'} · All months and years</p>
      <a class="btn sm" href="#/calendar">Open full calendar</a>
    </div>
  {:else if selectedDay}
    <div class="calendar-scope">
