<script>
  import { listTasks, resolveApprovalTask, resolveBulkProposals, retryDeadJob, dismissDeadJob } from '../lib/api.js'
  ...
  async function load() {
    ...
    try {
      ...
      onCount?.(tasks.length + proposalCards.length + jobs.length)
...
{:else}
  ...
  {#if tasks.length === 0 && proposalCards.length === 0 && jobs.length === 0}
