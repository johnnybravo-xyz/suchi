...
speak the same wire protocol. suchi's Phase-4 an existing DMS
compat surface will be verified by replaying real
`swift-bundle` / Bundle Mobile traffic against suchi and
asserting response shape. This tool captures that traffic while a
real an existing DMS is still running.

Nothing about the tool is Bundle-specific — it's a generic
...
| `--out` | `testdata/bundle-transcripts` | Fixture directory |
...
- Never commit `blobs/` content with real personal documents — record
  against a scratch Bundle with synthetic docs. `hack/emlfixtures`
...
- **Replay** side is Phase-4 work: reads the fixture set, hits suchi's
  ...
  will be `core/bundlecompat/contract_test.go`.
