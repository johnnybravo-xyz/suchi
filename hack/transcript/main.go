...
// case is capturing swift-paperless / Paperless Mobile traffic against
// a real Paperless-ngx so the compat surface can be replayed as
...
func main() {
	var (
		...
		target    = flag.String("target", "", "target Paperless-ngx URL (required)")
		outDir    = flag.String("out", "testdata/paperless-transcripts", "where to write fixtures")
	...
	if *target == "" {
		log.Fatalf("--target is required (the real Paperless-ngx URL)")
	}
	...
	// header rewrite quirk when tunneling to Cloudflare-fronted Paperless.
	...
	log.Printf("paperless-recorder listening on %s → %s (fixtures → %s)",
...
// allow-list-adjacent denylist because Paperless / mobile apps put
